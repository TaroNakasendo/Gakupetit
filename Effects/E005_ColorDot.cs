...
class E005_ColorDot : EffectBase, IEffect
{
    ...
    protected override Bitmap Masking(int v, Color color, Bitmap srcBitmap, Bitmap maskBitmap)
    {
        ...
        try
        {
            ...
            {
                gray = inRgbValues[j + 2];
                if (gray == 0) continue;
                if (rnd.Next(0, 255) < gray) outRgbValues[j + 0] = color.B;
                if (rnd.Next(0, 255) < gray) outRgbValues[j + 1] = color.G;
                if (rnd.Next(0, 255) < gray) outRgbValues[j + 2] = color.R;
            }
